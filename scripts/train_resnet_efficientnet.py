...
import time  # <-- added so benchmark_inference works
...
class CatsDogsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # return name so we can log misclassified examples
...
# -----------------------------------------------------
# Training Function WITH train/val + early stopping, ckpt
# -----------------------------------------------------
def train_model(model, model_name, df, device, version):
    ...
    # ----- Move model to device & pick trainable params -----
    ...
    if SCHED_CFG.get("type", "").lower() == "reduce_on_plateau":
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
            ...
            # verbose=True,  # ← removed, not supported in your PyTorch
        )
    ...
    final_val_labels = []
    final_val_preds = []
    final_val_probs = []  # probability of class "Dog" (1)
    misclassified = []
    ...
    for epoch in range(EPOCHS):
        ...
        # Save last epoch's val labels/preds/probs for metrics
        final_val_labels = all_val_labels
        final_val_preds = all_val_preds
        final_val_probs = all_val_probs

        # Collect misclassified examples (from last epoch)
        misclassified = []
    ...
    # ---------------------------------------------------------
    # LOAD BEST CHECKPOINT BEFORE METRICS + BENCHMARK
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    # PLOTS (train/val curves + confusion + ROC + PR)
    # ---------------------------------------------------------
    # accuracy plot
    ...
    acc_path_flat = os.path.join(SAVE_DIR, f"{model_name}_accuracy.png")
    acc_path_art = os.path.join(artifact_dir, "accuracy.png")
    plt.savefig(acc_path_flat)
    plt.savefig(acc_path_art)
    plt.close()
    ...
    loss_path_flat = os.path.join(SAVE_DIR, f"{model_name}_loss.png")
    loss_path_art = os.path.join(artifact_dir, "loss.png")
    plt.savefig(loss_path_flat)
    plt.savefig(loss_path_art)
    plt.close()

    # confusion matrix on validation set (last epoch)
    ...
    cm_path_flat = os.path.join(SAVE_DIR, f"{model_name}_confusion.png")
    cm_path_art = os.path.join(artifact_dir, "confusion.png")
    plt.savefig(cm_path_flat)
    plt.savefig(cm_path_art)
    plt.close()
    ...
    roc_path_art = os.path.join(artifact_dir, "roc.png")
    plt.savefig(roc_path)
    plt.savefig(roc_path_art)
    plt.close()
    ...
    pr_path_art = os.path.join(artifact_dir, "pr.png")
    plt.savefig(pr_path)
    plt.savefig(pr_path_art)
    plt.close()
    ...
    # ---------------------------------------------------------
    # Inference benchmark on best model
    # ---------------------------------------------------------
    ms_per_image = benchmark_inference(model, device, IMG_SIZE)

    # ---------------------------------------------------------
    # Save metrics + metadata + misclassified examples
    # ---------------------------------------------------------
    metrics = {
    ...
    # Return metrics for comparison plots at the end
    return {
        "train_acc": train_acc_list,
        "val_acc": val_acc_list,
        "train_loss": train_loss_list,
...
if __name__ == "__main__":
    ...
    res_cfg = MODELS_CFG["resnet18"]
    eff_cfg = MODELS_CFG["efficientnet_b0"]

    metrics_resnet = train_model(
        build_resnet(),
        res_cfg["name"],
        df,
        device,
    ...
    # -------------------------------------------------
    # COMPARISON PLOTS (for the last tab in Streamlit)
    # -------------------------------------------------
    epochs_resnet = range(1, len(metrics_resnet["val_acc"]) + 1)
    epochs_effnet = range(1, len(metrics_effnet["val_acc"]) + 1)

    # Accuracy comparison (validation)
    plt.figure(figsize=(6, 4))
    plt.plot(epochs_resnet, metrics_resnet["val_acc"], label="ResNet18 Val Acc")
    plt.plot(epochs_effnet, metrics_effnet["val_acc"], label="EfficientNet-B0 Val Acc")
    plt.xlabel("Epoch")
    plt.ylabel("Validation Accuracy")
    plt.legend()
    plt.title("ResNet18 vs EfficientNet-B0 – Val Accuracy")
    plt.savefig(os.path.join(SAVE_DIR, "compare_acc.png"))
    plt.close()

    # Loss comparison (validation)
    plt.figure(figsize=(6, 4))
    plt.plot(epochs_resnet, metrics_resnet["val_loss"], label="ResNet18 Val Loss")
    plt.plot(epochs_effnet, metrics_effnet["val_loss"], label="EfficientNet-B0 Val Loss")
    plt.xlabel("Epoch")
    plt.ylabel("Validation Loss")
    plt.legend()
    plt.title("ResNet18 vs EfficientNet-B0 – Val Loss")
    plt.savefig(os.path.join(SAVE_DIR, "compare_loss.png"))
    plt.close()

    # Confusion matrices side-by-side
    fig, axes = plt.subplots(1, 2, figsize=(9, 4))
    disp_res = ConfusionMatrixDisplay(metrics_resnet["cm"], display_labels=["Cat", "Dog"])
    disp_eff = ConfusionMatrixDisplay(metrics_effnet["cm"], display_labels=["Cat", "Dog"])
    disp_res.plot(ax=axes[0], colorbar=False)
    disp_eff.plot(ax=axes[1], colorbar=False)
    axes[0].set_title("ResNet18 – Val Confusion Matrix")
    axes[1].set_title("EfficientNet-B0 – Val Confusion Matrix")
    plt.tight_layout()
    plt.savefig(os.path.join(SAVE_DIR, "compare_cm.png"))
    plt.close()
